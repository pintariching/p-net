...
typedef enum pf_cmdev_state_values {
    ...
    PF_CMDEV_STATE_W_ARDYCNF,    /* Wait for app ready confirmation from
                                  * controller.
                                  */
...
typedef struct pf_rs_info {
    bool valid;
    uint8_t rs_alarm_transport_mode; /** Range 0 (read by record), 1 (using
                                        alarm reporting) */
} pf_rs_info_t;
...
typedef struct pf_alarm_cr_tag_header {
    ...
    uint8_t alarm_user_priority; /** Bits 12..15. Allowed: USER_PRIORITY_LOW
                                    (5), USER_PRIORITY_HIGH (6) */
...
typedef struct pf_alarm_cr_request {
    ...
    pf_alarm_cr_tag_header_t alarm_cr_tag_header_high; /** USER_PRIORITY_HIGH
                                                        * (6)
                                                        */
...
typedef struct pf_ppm {
    ...
    uint16_t send_clock_factor; /* Resolution: 31.25us, Allowed: 1..128,
                                   Default: 32 */
    uint16_t reduction_ratio;   /* Allowed: 1..512 */
...
typedef struct pf_ar {
    ...
    pf_session_info_t * p_sess; /* Incoming session, not the outgoing CControl
                                   session. Use pf_session_locate_by_ar() to
                                   find the CControl session. */
    ...
    bool alarm_enable;
    /* Alarm handling ALPMI/ALPMR machines: one for LOW (0) prio and one for
     * HIGH (1) prio. */
...
typedef struct pf_ppm_driver {
     ...
     *
     * Note that this is not from the PLC, but previously set by the
     * application.
     *
     * @param net              InOut: The p-net stack instance
     * @param iocr             InOut: The IOCR instance.
...
struct pnet {
    ...
    volatile pf_scheduler_timeouts_t scheduler_timeouts[16];
